...
impl Resampling {
    ...
    pub fn downsample_min_max(points: &[(f64, f64)], max_nb_points: usize) -> Vec<(f64, f64)> {
        ...
        assert!(
            max_nb_points >= 2,
            "we always keep first and last points"
        );
        #[cfg(not(tarpaulin_include))] // Wrongly marked uncovered.
        assert_eq!(
            max_nb_points % 2,
            0,
            "points always come in pairs (min/max)"
        );
    ...
    #[must_use]
    pub fn downsample_uniform(points: &[(f64, f64)], max_nb_points: usize, rng: &mut crate::random::Rng) -> Vec<(f64, f64)> {
        #[cfg(not(tarpaulin_include))] // Wrongly marked uncovered.
        assert!(
            max_nb_points >= 2,
            "we always keep first and last points"
        ...
        let sample = rng.sample(&points[1..points.len() - 1], max_nb_points -2);
        downsampled_points.extend(sample);
